(function(){
	...
	var Nomenclature = {
		...
		getQuadName500k: function(column, row, join) {
			...
			if (join == 4) {
				name += ',' + this.getQuadName1m(Math.floor((column + 2) / 2), Math.floor(row / 2), 1) + ' &ndash; ' + subquad + ',' + (subquad + 1);
			}
			return name;
		},
		...
		getQuadName050k: function(column, row, join) {
			...
			if (join == 4) {
				name += ',' + this.getQuadName100k(Math.floor((column + 2) / 2), Math.floor(row / 2), 1) + ' &ndash; ' + subquad + ',' + (subquad + 1);
			}
			return name;
		},
	...
	L.SovietTopoGrid = L.LayerGroup.extend({
		...
		_addQuad: function(bounds, id, title, color, layer) {
			...
			var rect_options = {smoothFactor: 0,
											noClip: true,
											clickable: false,
											fill: false,
